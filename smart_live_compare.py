...
def setup_logger(log_path: str, verbose: bool) -> logging.Logger:
    ...
    fh = logging.FileHandler(log_path, mode="w", encoding="utf-8"); fh.setLevel(logging.DEBUG); fh.setFormatter(fmt); logger.addHandler(fh)
    return logger
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--allow-missing-cols", action="store_true", help="If live-tail misses columns, fill by train medians.")
    p.add_argument("--diff-topk", type=int, default=8, help="Top-K features to log on mismatch (live-tail vs baseline).")

    # live-tail defaults
    p.add_argument("--predict-drop-last", action="store_true", help="Drop last row after feature build in live-tail (recommended).")
    ...
    p.add_argument("--auto-tail-samples", type=int, default=3, help="Samples (cutoffs) to probe for auto-tail.")
...
def load_raw_csvs(args) -> Dict[str, pd.DataFrame]:
    ...
    for tf, path in base_csvs.items():
        if not os.path.isfile(path):
            if tf == "30T": raise FileNotFoundError(f"Main TF (30T) is required. Missing: {path}")
            continue
        raw_df[tf] = expect_cols(pd.read_csv(path))
    if "30T" not in raw_df: raise FileNotFoundError("30T dataframe missing; abort.")
    return raw_df
...
def load_train_medians(model_dir: str, payload: dict) -> Optional[dict]:
    ...
    if not os.path.isabs(path): path = os.path.join(model_dir, path)
...
def write_tail_csvs(raw_df: Dict[str, pd.DataFrame], symbol: str, cutoff: pd.Timestamp, out_dir: str, hist_rows: Dict[str, int]) -> Dict[str, str]:
...
def _test_tail_once(filepaths: Dict[str, str], window: int, final_cols: List[str], predict_drop_last: bool) -> Tuple[bool, int]:
    prep = PREPARE_DATA_FOR_TRAIN(filepaths=filepaths, main_timeframe="30T", verbose=False, fast_mode=True, strict_disk_feed=True)
    merged = prep.load_data()
    if merged.empty: return (False, 0)
    X, _, _, _, _ = prep.ready(merged, window=window, selected_features=final_cols, mode="predict", with_times=True, predict_drop_last=predict_drop_last)
    if X.empty: return (False, len(final_cols))
    miss = list(set(final_cols) - set(X.columns))
    return (len(miss) == 0, len(miss))

def auto_tail_search(log: logging.Logger, raw_df: Dict[str, pd.DataFrame], symbol: str, final_cols: List[str], window: int,
                     predict_drop_last: bool, *, samples: int = 3, max30t: int = 6000,
                     mult_1h: float = 0.5, mult_15t: float = 2.0, mult_5t: float = 6.0) -> Dict[str, int]:
    log.info("[auto-tail] start: samples=%d max30t=%d multipliers(1H=%.2f,15T=%.2f,5T=%.2f)", samples, max30t, mult_1h, mult_15t, mult_5t)
    main = raw_df["30T"]; N = len(main)
    ...
    try:
        for ci, cutoff in enumerate(cutoffs, 1):
            low, high = 64, min(max30t, N-2); ok_high = None
            # doubling to find an OK upper bound
            base = low
            while base <= high:
            ...
            while lo <= hi:
                ...
                fps = write_tail_csvs(raw_df, symbol, cutoff, it_dir, tails)
                if "30T" not in fps:
                    shutil.rmtree(it_dir, ignore_errors=True); lo = mid + 1; continue
                ok, _ = _test_tail_once(fps, window, final_cols, predict_drop_last)
                shutil.rmtree(it_dir, ignore_errors=True)
                if ok: best = mid; hi = mid - 1
                else: lo = mid + 1
    ...
    log.info("[auto-tail] chosen tails: 30T=%d 1H=%d 15T=%d 5T=%d", final["30T"], final["1H"], final["15T"], final["5T"])
    return final
...
def run_replay_and_predict_baseline(args, log, payload, pipeline, window, neg_thr, pos_thr, final_cols):
    ...
    # --- replay (mode=train) : TRAIN-like
    X_tr, y_tr, _, _ = prep.ready(merged, window=window, selected_features=final_cols,
                                  mode="train", predict_drop_last=False, train_drop_last=False)
    if not X_tr.empty and all(c in X_tr.columns for c in final_cols): X_tr = X_tr[final_cols]
    probs_tr = pipeline.predict_proba(X_tr)[:,1] if len(X_tr) else np.array([], dtype=float)
    preds_tr = np.full(len(probs_tr), -1, dtype=int)
    preds_tr[probs_tr <= neg_thr] = 0; preds_tr[probs_tr >= pos_thr] = 1
    mask_tr = preds_tr != -1
    ...
    # save CSV
    df_tr = pd.DataFrame({
        "prob": probs_tr, "pred": preds_tr, "true": (y_tr.values if len(y_tr) else [])
    })
    out_tr = os.path.join(args.model_dir, "smart_replay_train.csv"); df_tr.to_csv(out_tr, index=False)

    log.info("[replay/train] total=%d predicted=%d unpred=%d acc=%.4f%%",
             len(preds_tr), int(mask_tr.sum()), int((~mask_tr).sum()), acc_tr)

    # --- baseline-predict (mode=predict + drop-last) : for exact LIVE comparisons
    Xp, _, _, _, t_idx = prep.ready(merged, window=window, selected_features=final_cols,
                                    mode="predict", with_times=True, predict_drop_last=True)
    if not Xp.empty and all(c in Xp.columns for c in final_cols): Xp = Xp[final_cols]
    probs_p = pipeline.predict_proba(Xp)[:,1] if len(Xp) else np.array([], dtype=float)
    preds_p = np.full(len(probs_p), -1, dtype=int)
    preds_p[probs_p <= neg_thr] = 0; preds_p[probs_p >= pos_thr] = 1
    ...
    y_true_list = []
    for tt in t_idx:
        if tt in time_to_idx.index:
            mi = int(time_to_idx.loc[tt])
            if mi+1 < len(mclose):
            ...
            y_true_list.append(np.nan)
    y_true_p = pd.Series(y_true_list).astype("float").dropna().astype(int)
    # align lengths (drop NaNs)
    valid_mask = ~pd.isna(y_true_list)
    t_idx_p = t_idx[valid_mask.values] if len(t_idx) else pd.Series([], dtype="datetime64[ns]")
    probs_p_v = probs_p[valid_mask.values] if len(probs_p) else np.array([], dtype=float)
    preds_p_v = preds_p[valid_mask.values] if len(preds_p) else np.array([], dtype=int)
    ...
    out_p = os.path.join(args.model_dir, "smart_replay_predict.csv"); df_p.to_csv(out_p, index=False)

    log.info("[baseline/predict] total=%d predicted=%d unpred=%d acc=%.4f%%",
             len(preds_p_v), int(mask_p.sum()), int((~mask_p).sum()), acc_p)

    # برگردان برای مقایسه‌های بعدی
    return {
        "prep": prep,
        "merged": merged,
...
def run_incremental(args, log, baseline, pipeline, neg_thr, pos_thr, final_cols):
    t_idx = baseline["t_idx"]; probs_all = baseline["probs_p"]; preds_all = baseline["preds_p"]; y_true_all = baseline["y_true_p"]
    if len(t_idx) == 0:
        log.error("[incremental] empty baseline t_idx; abort."); return
    ...
    for k, j in enumerate(range(start, end+1), start=1):
        prob = float(probs_all[j]); pred = decide(prob, neg_thr, pos_thr); true = int(y_true_all.iloc[j])
        if pred == -1:
        ...
        log.info("[INC %5d/%5d] time=%s prob=%.3f → pred=%s true=%d → %s | P=%d W=%d L=%d U=%d Acc=%.2f%% Cov=%.2f%% | BUY=%d SELL=%d NONE=%d",
                 k,total,str(t_idx.iloc[j]),prob,dec_label,true,verdict,preds,wins,losses,unpred,acc,cov,buy_n,sell_n,none_n)
        rows.append({"iter":k,"time":t_idx.iloc[j],"prob":prob,"pred":int(pred),"true":int(true),"verdict":verdict})
    out_inc = os.path.join(args.model_dir, "smart_incremental.csv"); pd.DataFrame(rows).to_csv(out_inc, index=False)
...
def run_live_tail(args, log, payload, pipeline, window, neg_thr, pos_thr, final_cols, baseline):
    raw_df = load_raw_csvs(args); main_df = raw_df["30T"]
    # hist rows
    if args.auto_tail:
        tails = auto_tail_search(log, raw_df, args.symbol, final_cols, window, args.predict_drop_last,
                                 samples=max(1,int(args.auto_tail_samples)), max30t=int(args.auto_tail_max30t),
    ...
    # build time→row index map for baseline predict
    ...
    try:
        for k in range(total_iters):
            ...
            if missing:
                if not args.allow-missing-cols:
                    shutil.rmtree(iter_dir, ignore_errors=True)
                    log.error("[live/fatal] missing %d train cols at cutoff=%s; ex: %s", len(missing), str(cutoff), missing[:10])
                    return
                if train_medians is None:
                    shutil.rmtree(iter_dir, ignore_errors=True)
                    log.error("[live/fatal] --allow-missing-cols set but train medians file missing."); return
                X = X.reindex(columns=want, fill_value=np.nan)
                for c in missing:
                    mv = train_medians.get(c, None)
                    if mv is None or not np.isfinite(mv):
                        shutil.rmtree(iter_dir, ignore_errors=True)
                        log.error("[live/fatal] no median for column %s", c); return
                    X[c] = float(mv)
                X = X.fillna(0.0)
            ...
            # last feature timestamp in this live chunk
            ...
            # compare with baseline (if timestamp available)
            base_prob = base_pred = base_true = np.nan
            l2 = max_abs = np.nan
            diff_cols = 0
            topk_txt = ""
            if t_feat is not None and t_feat in base_time_to_idx.index:
                bj = int(base_time_to_idx.loc[t_feat])
                base_prob = float(base_probs[bj])
                base_pred = int(base_preds[bj])
                base_true = int(base_ytrue.iloc[bj])
                # feature diffs:
                try:
                    row_live = X.tail(1).iloc[0]; row_base = base_X.iloc[bj]
                    # ensure same order:
            ...
            # if pred mismatch vs baseline, print top diffs
    ...
    out_live = os.path.join(args.model_dir, "smart_live_tail.csv"); pd.DataFrame(rows).to_csv(out_live, index=False)
...
def main():
    ...
    # 2) INCREMENTAL (must match baseline/predict row-by-row)
    run_incremental(args, log, baseline, pipeline, neg_thr, pos_thr, final_cols)

    # 3) LIVE-TAIL (+ auto-tail if requested) with per-iteration diffs vs baseline
