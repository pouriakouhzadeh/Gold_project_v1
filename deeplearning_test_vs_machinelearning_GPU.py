...
# ==============================
# قفل‌کردن GPU قبل از import torch
# ==============================
import os, sys, argparse

parser = argparse.ArgumentParser()
parser.add_argument("--gpu", type=int, default=0, help="اندیس GPU داخل کانتینر (معمولاً 0)")
parser.add_argument("--batch", type=int, default=512, help="اندازه‌ی Batch برای آموزش/پیش‌بینی")
args, _ = parser.parse_known_args()

# برای Polaris (RX 580 / gfx803) و پایداری ROCm:
...
# ==============================
# Logging (console + file, هر بار از اول)
# ==============================
LOG_FILE = "ensemble_deep.log"
# FileHandler با mode='w' باعث می‌شود لاگ هر بار از ابتدا نوشته شود
logging.basicConfig(
    level=logging.INFO,
...
# ==============================
# GPU: فقط تک‌GPU، بدون fallback به CPU
# ==============================
if not torch.cuda.is_available():
    log.info("HIP ver: %s | cuda.is_available=%s", getattr(torch.version, "hip", None), torch.cuda.is_available())
    sys.exit("❌ GPU در دسترس نیست. کانتینر/متغیرهای ROCR_VISIBLE_DEVICES و دسترسی /dev/kfd و render node را بررسی کنید.")

torch.cuda.set_device(0)  # چون کانتینر فقط یک GPU می‌بیند
device = torch.device("cuda:0")
log.info("HIP ver: %s | device=%s | name=%s",
         getattr(torch.version, "hip", None), device, torch.cuda.get_device_name(0))
...
def make_loader(X, y, batch=512, shuffle=False):
    # برای پایداری روی ROCm داخل داکر:
...
def train_one(model, X_tr, y_tr, X_val, y_val,
    ...
    # Warm-up کوچک برای پایدار شدن HIP
    model.train()
    dummy_x = torch.zeros((4, X_tr.shape[1]), dtype=torch.float32, device=device)
    dummy_y = torch.zeros((4,), dtype=torch.float32, device=device)
    loss = crit(model(dummy_x), dummy_y); loss.backward(); model.zero_grad(set_to_none=True)
...
def main():
    ...
    for name, ctor, grid in MODELS:
        def model_ctor(**p): return ctor(**p)
    ...
    if results:
        ...
        for r in results:
            ...
            votes.append(y_c); probas.append(proba)
