...
class AdaptiveThresholds:
    def __init__(self, base_neg: float, base_pos: float, enabled: bool,
                 buf_len:int=1000, q_low=0.10, q_high=0.90, warm_min:int=500, fallback_soft=(0.30, 0.70)):
        ...
        if self.enabled:
            ...
            self.buf = deque(maxlen=self.buf_len)
        else:
            self.buf = None
...
def main():
    ...
    ap.add_argument("--adaptive", action="store_true",
                    help="اگر ست شود، آستانه‌های تطبیقی فعال می‌شوند. برای همسانی با تست batch، خاموش بگذار.")
    args = ap.parse_args()
    ...
    ath = AdaptiveThresholds(neg0, pos0, enabled=bool(args.adaptive))

    # هیستوگرام ساده جهت پایش
    ...
    while True:
        ...
        try:
            # STRICT: دقیقا مثل پروداکشن واقعی (هیچ دست‌کاری اضافه)
            ...
            # پاکسازی CSVهای live بعد از تولید پاسخ
            for p in list(live_paths.values()):
                try: Path(p).unlink(missing_ok=True)
                except Exception: pass
