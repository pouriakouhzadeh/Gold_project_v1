...
# -*- coding: utf-8 -*-
"""
Realistic disk-feed live simulator (STRICT parity with batch)
"""
...
def _smart_load_model(path: str):
    ...
    try:
        with open(path, "rb") as f:
            head = f.read(2)
        if head == b"\x1f\x8b":
            import gzip;  return pickle.load(gzip.open(path, "rb"))
                ...
                if kk in obj:
                    ...
                    if isinstance(v, str):
                        for parser in (json.loads, ast.literal_eval):
                            try:
                                vv = parser(v); los = _list_of_strings(vv)
                                if los: return los
...
def recover_thresholds_and_window(search_dirs: List[str], log: logging.Logger) -> Tuple[Optional[float], Optional[float], Optional[int]]:
            ...
            for k in _THR_KEYS:
                for kk in (k,k.upper(),k.lower()):
                    if kk in obj and isinstance(obj[kk], (int,float)):
                        if "neg" in kk or "negative" in kk: neg = float(obj[kk])
                        if "pos" in kk or "positive" in kk: pos = float(obj[kk])
            for k in _WIN_KEYS:
                for kk in (k,k.upper(),k.lower()):
    ...
    if any(v is not None for v in (neg,pos,win)):
        log.info("Recovered thresholds/window → neg=%s pos=%s win=%s",
...
def infer_window_from_cols(cols: List[str]) -> int:
    import re; r = re.compile(r"_tminus(\d+)$"); mx=0
    for c in cols:
        m=r.search(str(c));  mx=max(mx, int(m.group(1))) if m else mx
...
def coerce_pipeline(obj: Any) -> Any:
    ...
    if isinstance(obj, dict):
        for k in ("pipeline","model","estimator","clf","sk_pipeline"):
            if k in obj and hasattr(obj[k],"predict_proba"): return obj[k]
    raise TypeError("Loaded object is not a sklearn estimator/pipeline with predict_proba.")

# ---------------- Time helpers ----------------
def timeframe_delta(tf: str) -> timedelta:
    tf=tf.upper()
    return timedelta(minutes=30) if tf in ("30T","M30") else (
           timedelta(minutes=15) if tf in ("15T","M15") else (
           timedelta(minutes=5)  if tf in ("5T","M5")   else (
           timedelta(hours=1)    if tf in ("1H","H1")   else timedelta(minutes=30))))

def compute_gt_from_m30(m30: pd.DataFrame, t_feat: pd.Timestamp) -> Optional[int]:
    if pd.isna(t_feat): return None
    t = pd.to_datetime(m30["time"])
    pos = t.searchsorted(t_feat, side="left")
    if pos>=len(m30): return None
    if t.iloc[pos] != t_feat:
        pos -= 1
        if pos < 0 or t.iloc[pos] > t_feat: return None
    if pos+1 >= len(m30): return None
    c0 = float(m30.iloc[pos]["close"]); c1 = float(m30.iloc[pos+1]["close"])
...
def load_base_csvs(base_dir: str, symbol: str) -> Dict[str, pd.DataFrame]:
    ...
    for tf,p in fps.items():
        if not os.path.isfile(p): raise FileNotFoundError(f"CSV not found for {tf}: {p}")
        df = pd.read_csv(p)
        if "time" not in df.columns: raise ValueError(f"'time' column missing in {tf} CSV")
        df["time"] = pd.to_datetime(df["time"], errors="coerce")
        df = df.dropna(subset=["time"]).sort_values("time").reset_index(drop=True)
        dfs[tf] = df
    return dfs

def mt4_name(tf: str) -> str:
    return "M30" if tf=="30T" else ("M15" if tf=="15T" else ("M5" if tf=="5T" else "H1"))

def write_initial_tmp(tmp_dir: str, dfs: Dict[str, pd.DataFrame], t_hist_start: pd.Timestamp, t0: pd.Timestamp):
    # STRICT: فقط تا «آخرین کندل بسته قبل از t0» (یعنی time < t0)
    for tf, df in dfs.items():
        mask = (df["time"] >= t_hist_start) & (df["time"] < t0)
        out = df.loc[mask].copy()
        out.to_csv(os.path.join(tmp_dir, f"XAUUSD_{mt4_name(tf)}.csv"), index=False)

def append_new_rows(tmp_dir: str, base_df: pd.DataFrame, tf: str,
                    last_written_time: Optional[pd.Timestamp],
                    cutoff: pd.Timestamp) -> Optional[pd.Timestamp]:
    # STRICT: فقط ردیف‌هایی با time < cutoff را اضافه کن (نه <=)
    if last_written_time is None:
        mask = base_df["time"] < cutoff
    else:
        mask = (base_df["time"] > last_written_time) & (base_df["time"] < cutoff)
    new = base_df.loc[mask]
    if new.empty: return last_written_time
    out_path = os.path.join(tmp_dir, f"XAUUSD_{mt4_name(tf)}.csv")
    mode = "a" if os.path.exists(out_path) else "w"
    header = not os.path.exists(out_path) if mode=="a" else True
    new.to_csv(out_path, mode=mode, header=header, index=False)
    return pd.Timestamp(new.iloc[-1]["time"])

# --------------------------- main ---------------------------
def main():
    ap = argparse.ArgumentParser("Strict disk-feed live simulator + batch parity")
    ap.add_argument("--mode", default="real", choices=["real"])
    ap.add_argument("--split", default="tail", choices=["ga","tail"])
    ap.add_argument("--n-test", type=int, default=4000)
    ap.add_argument("--history-bars", type=int, default=2500)
    ap.add_argument("--fast-mode", type=int, default=1)
    ap.add_argument("--audit", type=int, default=50)
    ...
    if isinstance(obj, dict):
        for k in _COLS_KEYS:
            if k in obj:
                model_side_cols = _list_of_strings(obj[k]);  break
    pipeline = coerce_pipeline(obj)

    # Columns
    cols = model_side_cols or recover_cols_from_train_distribution([os.path.dirname(args.model), os.getcwd()], log)
    if not cols: raise ValueError("Could not obtain train_window_cols")
    seen=set(); train_window_cols=[str(c) for c in cols if not (str(c) in seen or seen.add(str(c)))]

    # Window
    window = args.window if (args.window and args.window>0) else infer_window_from_cols(train_window_cols)

    # Thresholds
    neg_thr, pos_thr = args.neg_thr, args.pos_thr
    if int(args.use_model_thresholds)==1:
        n2,p2,w2 = recover_thresholds_and_window([os.path.dirname(args.model), os.getcwd()], log)
        if neg_thr is None and n2 is not None: neg_thr=float(n2)
        if pos_thr is None and p2 is not None: pos_thr=float(p2)
        if (args.window==0) and (w2 is not None) and (w2>0): window=int(w2)
    if neg_thr is None: neg_thr = 0.005
    if pos_thr is None: pos_thr = 0.990
    ...
    m30 = base["30T"]; all_times_30 = pd.to_datetime(m30["time"]).sort_values()
    if args.split=="ga":
        total=len(all_times_30); train_end=int(total*0.85); thresh_end=int(total*0.90)
    ...
    write_initial_tmp(args.tmp_dir, base, t_hist_start, t0)  # STRICT: < t0 (نه <=)
    ...
    prep = PREPARE_DATA_FOR_TRAIN(filepaths=tmp_filepaths, main_timeframe="30T",
    ...
    for step, t_cur in enumerate(anchors, start=1):
        ...
        # STRICT feed: فقط time < t_cur را وارد کن (همه TFها)
        for tf in ("30T","1H","15T","5T"):
            last_written[tf] = append_new_rows(args.tmp_dir, base[tf], tf, last_written.get(tf), cutoff=t_cur)

        # یک‌بار پردازش اندیکاتورها روی وضعیت فعلی CSVها
        raw_all = prep.load_data()

        # فقط یک دمِ کوتاه از tail کافی‌ست (برای سرعت) — فیچرِ پایدار را می‌گیریم
        tail_len = max(window+3, 12)
        data_window = raw_all.tail(tail_len)

        # نـکـتـهٔ طـلایـی: feed پایدار است ⇒ دیگر drop-last لازم نیست
        X_tail, feats, t_feat = prep.ready_incremental(
            data_window,
            window=window,
        ...
        if X_tail.empty or (t_feat is None):
        ...
        # ورودی دقیقاً با ستون‌های Train
        X_in = (X_tail
                .reindex(columns=train_window_cols, fill_value=0.0)
        ...
        gt = compute_gt_from_m30(m30, pd.Timestamp(t_feat))
        ...
        if (gt is None) or (pred == -1):
            ...
            live_times_decided.append(pd.Timestamp(t_feat))

        decided = wins + loses
        acc = (wins/decided) if decided>0 else float("nan")

        if int(args.audit)==1 or (args.audit and (step % int(args.audit)==0)):
            exp_t_feat = t_cur - dt30  # فقط برای sanity check
            log.info("[ALIGN] t_cur=%s | prep_t_feat=%s | exp_t_feat=%s | equal=%s",
                     str(t_cur)[:16], str(t_feat)[:16], str(exp_t_feat)[:16],
                     "YES" if pd.Timestamp(t_feat)==exp_t_feat else "NO")
        ...
        if args.save_csv:
            rows_csv.append({
                "step": step, "t_cur": str(t_cur), "t_feat": str(t_feat),
                "proba": p, "pred": pred, "gt": (None if gt is None else int(gt)), "outcome": outcome
            })
    ...
    # Batch parity روی همان بازه
    t_min = pd.Timestamp(anchors[0]); t_max = pd.Timestamp(anchors[-1])
    ...
    mask = (pd.to_datetime(raw_all_b[tcol]) >= t_min) & (pd.to_datetime(raw_all_b[tcol]) <= t_max)
    test_slice = raw_all_b.loc[mask].copy()

    Xb, yb, _, _, t_idx_b = prep_batch.ready(
        test_slice, window=window, selected_features=train_window_cols,
        mode="train", with_times=True, predict_drop_last=False
    )
    ...
    pred_b[prob_b <= neg_thr] = 0; pred_b[prob_b >= pos_thr] = 1
    ...
    # Batch روی همان timestampهایی که لایو تصمیم گرفته
    ...
    if idxs:
        pred_b_sub = pred_b[idxs]; yb_sub = yb.iloc[idxs]
