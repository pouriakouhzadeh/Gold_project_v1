...
# -*- coding: utf-8 -*-
"""
deploy_ready_guard.py
---------------------
گارد دپلوی: تضمین برابری LIVE با baseline (از نظر پیش‌بینی و خودِ فیچرها) + چک اختیاری همسانی Acc با TRAIN.
- baseline: fast_mode=True, strict_disk_feed=True, predict_drop_last=True
- auto-tail: علاوه بر حضور ستون‌ها، همگرایی عددی (L2/MaxAbs) با baseline را نیز تضمین می‌کند.

خروجی‌ها (داخل --model-dir):
- deploy_baseline.csv   (baseline/predict_strict + y_true هم‌تراز)
- deploy_live.csv       (شبیه‌سازی واقعی tail به‌ازای هر cutoff)
- deploy_guard.log      (لاگ کامل)
Exit code:
- 0 → READY
- 2 → NOT READY
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Deploy Guard: parity LIVE vs baseline with numeric convergence.")
...
def build_baseline_predict_strict(args, log, pipeline, window, final_cols):
    ...
    # thresholds come from saved payload (neg_thr,pos_thr)
...
def auto_tail_search_with_convergence(
    ...
    *,
    ...
    log.info("[auto-tail] (convergence) start: samples=%d max30t=%d", samples, max30t)
    main = raw_df["30T"]
    N = len(main)
...
def main():
    ...
    log.info("=== deploy_ready_guard starting ===")
    ...
    log.info("=== deploy_ready_guard finished ===")
